      ...
      form.find('input, select, textarea').each(function () {
        ...
        if (i.data('criteria')) {
          ...
          if (val !== undefined && val !== "" && (typeof val != "number" || !isNaN(val))) {
            ...
            addCriteria(i.data('criteria'), i.attr('name'), val);
...
function setDataToForm(o, f){
    ...
    for (var i in o.attributes){
      if (f.find('[name="' + i + '"]').is('[type="date"]'))
      f.find('[name="' + i + '"]')[0].valueAsDate = o.attributes[i];
      else if (f.find('[name="' + i + '"]').is(':checkbox'))
      f.find('[name="' + i + '"]').prop('checked',o.attributes[i] );
      else
      f.find('[name="' + i + '"]').val(decodeEntities(o.attributes[i]));
...
function simblaFormSubmit(form) {
  ...
  function error(err) {
    ...
    T.append('<div class="text-danger simbla-form"><label>' + ( typeof err == "object" ? err.err || err.responseText :"") + T.data('failure') + '</label></div>');
    else if (T.data('failure-type') == 'link') {
      window.location = T.data('failure')
    }

    if (err.responseText.toUpperCase().indexOf('CAPTCHA'))
    grecaptcha.reset(T.data('captcha'));
  }
...
$(document).on('click', '.db-form-add', function (){
 ...
 switch(viewOption) {
  ...
  obj.fetch().then(function (e){setDataToForm(e, form);})
  ...
  showAlert('Are you sure you want to delete this object?', 'Delete', function (){
    var tr = T.closest('tr');
    var obj = tr.data('val');
    obj.destroy().then(function (){tr.remove()}).fail (showAlert)
  });
});
 ...
 obj.fetch(function (o){
  ...
  if ($(window).width() > 768){
      ...
      for (var i in dataGroups)
        $('[data-group-id="' +  i + '"]').height(dataGroups[i]);
  }
});
